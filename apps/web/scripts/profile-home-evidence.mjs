// Regenerates the two packs the profile-home change touches: the rider
// profile (now a welcome home) and the map home (now carrying the profile
// chip and the four tab bar), at 360px, day and night, English and Shona,
// into docs/design-evidence/{profile,home}/. Headed Chromium (the MapLibre
// canvas paints blank in headless). Expects the dev server on :3000 with
// E2E_AUTH=on and the demo pool seeded.
//
// Usage: node scripts/profile-home-evidence.mjs   (from apps/web)
...
// Two cleared fares so the welcome stats read warm (total, this month, top
// trip), plus prefs and emergency details so the settings group is populated.
async function prepareFixtures() {
  ...
  // two rides on the same corridor pair: total 2, this month 2, one top trip
  for (let i = 0; i < 2; i += 1) {
    ...
    if (buyErr) throw new Error(`ticket ${i} failed: ${buyErr.message}`);
    ...
    if (redeemErr || redeemed[0]?.outcome !== "success") {
      throw new Error(
        `redeem ${i} failed: ${redeemErr?.message ?? redeemed[0]?.outcome}`,
      );
    }
  }
}
...
async function shot(page, screen, variant, fullPage) {
...
async function main() {
  ...
  for (const variant of VARIANTS) {
    ...
    // the map home, showing the profile chip (top right) and the four tab bar
    await page.goto(`${BASE}/app`);
    await settleMap(page);
    await shot(page, "home", variant, false);

    // the profile welcome home, full page
    await page.goto(`${BASE}/app/profile`);
    await page.waitForTimeout(1_000);
    await shot(page, "profile", variant, true);

    await context.close();
  }

  await browser.close();
  console.log("profile and home evidence written to docs/design-evidence");
}
